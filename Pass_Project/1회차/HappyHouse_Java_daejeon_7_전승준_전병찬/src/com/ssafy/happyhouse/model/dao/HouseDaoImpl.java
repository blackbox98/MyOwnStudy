	...
	public HouseDeal search(int no) {
		...
		for(int i=0; i<search.size(); i++) {
			if(search.get(i).getNo() == no) {
				HouseInfo houseIn = houseInfo.get(search.get(i).getDong() + search.get(i).getAptName());
				if (houseIn != null) {
					search.get(i).setBuildYear(houseIn.getBuildYear());
					search.get(i).setCode(houseIn.getCode());
					search.get(i).setJibun(houseIn.getJibun());
					search.get(i).setImg(houseIn.getImg());
				}
				return search.get(i);
			}
		}
